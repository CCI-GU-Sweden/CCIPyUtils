...
class CCILogger:
    ...
    def setup_logger(cls, log_file_name: str, app_name: str, level=logging.DEBUG):
        ...
        fmt_str = '%(process)d: %(asctime)s -%(levelname)s-: %(message)s'

        logging.basicConfig(filename=log_file_name, level=level,
                            format=fmt_str)
        local_logger = logging.StreamHandler(sys.stdout)
        ll_fmt = CustomFormatter()
        local_logger.setFormatter(ll_fmt)
        local_logger.setLevel(level)
        logging.getLogger(app_name).addHandler(local_logger)
