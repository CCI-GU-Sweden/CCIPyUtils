...
def remove_rois_from_image_by_description(omero_conn: OmeroConnection, image_id: int, roi_description: str) -> int:
    """Remove all ROIs from an OMERO image.
    Args:
        omero_conn (OmeroConnection): OMERO connection object.
        image_id (int): ID of the OMERO image.
    ...
    with OmeroGetterCtx(omero_conn) as getter:
        ...
        removed = 0
        for roi in rois:
            if roi.getDescription() == rstring(roi_description):
                us.deleteObject(roi)
                removed += 1
...
def remove_rois_from_dataset_by_description(omero_conn: OmeroConnection, dataset_id: int, roi_description: str) -> int:
    """Remove all ROIs from all images in an OMERO dataset.
    Args:
        omero_conn (OmeroConnection): OMERO connection object.
        dataset_id (int): ID of the OMERO dataset.
    ...
    with OmeroGetterCtx(omero_conn) as getter:
        ...
        tot_removed = 0
        for image_id in image_ids:
            tot_removed += remove_rois_from_image_by_description(omero_conn, image_id, roi_description)
