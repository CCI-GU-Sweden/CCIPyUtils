...
class OmeroConnection:
    ...
    def _close_omero_connection(self, hard_close=False):
    ...
    def get_dataset(self, data_set_id: int) -> DatasetWrapper | None:
        return self._get_object("Dataset", data_set_id)
    ...
    def create_tag_annotation(self, tag_value):
        with self._mutex:
            CCILogger.info(f"Creating tag {tag_value}")
    ...
    def set_annotation_on_image(self, image, annotation):
        with self._mutex:
            try:
                CCILogger.info(f"linking tag {annotation.getValue()} to image {image.getId()}")
                image.linkAnnotation(annotation)
            except omero.ValidationException as e:  # pyright: ignore[reportAttributeAccessIssue]
                CCILogger.warning(f"Failed to insert the tag {annotation.getValue()} to image {image}: {str(e)}")
